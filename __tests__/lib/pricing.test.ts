...
describe("pricing", () => {
  ...
  it("applies 1-bedroom tiered rates for a 4-night stay", () => {
    const rack = 120_000;
    const nights = 4;
    const quote = computeBookingQuote(rack, "2026-03-20", "2026-03-24");
    expect(quote).not.toBeNull();
    ...
    expect(quote.nights).toBe(nights);
    expect(quote.discountAmount).toBe(20_000 * nights);
    expect(quote.effectiveNightlyRateNgn).toBe(100_000);
    expect(quote.accommodationTotalNgn).toBe(100_000 * nights);
  });
  ...
  it("exposes one-bedroom marketing tiers from rack rate", () => {
    const tiers = getStayDiscountTiers(120_000);
    expect(tiers[0]?.effectiveNightlyRateNgn).toBe(120_000);
    expect(tiers[1]?.effectiveNightlyRateNgn).toBe(110_000);
    expect(tiers[2]?.effectiveNightlyRateNgn).toBe(100_000);
    expect(tiers[3]?.effectiveNightlyRateNgn).toBe(90_000);
...
describe("stay discount tiers", () => {
  it("returns zero discount for a single night", () => {
    ...
    expect(getEffectiveNightlyRate(120_000, 1)).toBe(120_000);
  });
  ...
  it("returns ₦10k/night off for 2-night stays", () => {
    ...
    expect(getEffectiveNightlyRate(120_000, 2)).toBe(110_000);
  });
  ...
  it("returns ₦20k/night off for 3–6 nights", () => {
    ...
    expect(getEffectiveNightlyRate(120_000, 4)).toBe(100_000);
  });

  it("returns ₦30k/night off for 7–21 nights", () => {
    expect(getStayDiscountPerNight(14)).toBe(30_000);
    expect(getEffectiveNightlyRate(250_000, 14)).toBe(220_000);
    expect(getEffectiveNightlyRate(120_000, 14)).toBe(90_000);
  });
  ...
  it("returns ₦40k/night off for 28+ nights", () => {
    ...
    expect(getEffectiveNightlyRate(120_000, 30)).toBe(80_000);
  });
});
