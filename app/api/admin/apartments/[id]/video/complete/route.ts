import { NextRequest, NextResponse } from "next/server";
import { requireAdmin } from "@/lib/admin/auth";
import {
    completeApartmentVideoDirectUpload,
    serializeApartmentVideo,
} from "@/lib/admin/apartmentVideo";
import { parseJsonBody } from "@/lib/validation/http";
import { z } from "zod";

export const maxDuration = 300;

type RouteError = {
    httpResponse?: Response;
    statusCode?: number;
};

type RouteContext = {
    params: Promise<{ id: string }>;
};

const completeUploadBodySchema = z
    .object({
        videoId: z.string().trim().min(1),
        mimeType: z.string().trim().min(1).max(100),
    })
    .strict();

function routeErrorResponse(err: unknown) {
    const statusCode = (err as { statusCode?: number }).statusCode ?? 500;
    const message =
        err instanceof Error ? err.message : "Unexpected server error";
    return NextResponse.json({ error: message }, { status: statusCode });
}

export async function POST(request: NextRequest, context: RouteContext) {
    try {
        await requireAdmin(request, ["admin"]);
    } catch (err) {
        const { httpResponse, statusCode } = err as RouteError;
        if (httpResponse) return httpResponse;
        const status = statusCode ?? 401;
        return NextResponse.json(
            { error: status === 403 ? "Forbidden" : "Unauthorized" },
            { status },
        );
    }

    const parsed = await parseJsonBody(request, completeUploadBodySchema);
    if (!parsed.success) return parsed.response;

    const { id: apartmentId } = await context.params;

    try {
        const row = await completeApartmentVideoDirectUpload({
            apartmentId,
            videoId: parsed.data.videoId,
            mimeType: parsed.data.mimeType,
        });
        return NextResponse.json({
            ok: true,
            apartmentVideo: serializeApartmentVideo(row),
        });
    } catch (err) {
        console.error("Failed to complete apartment video upload:", err);
        return routeErrorResponse(err);
    }
}
